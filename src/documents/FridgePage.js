...
import '../stylesheets/fridgePage.css'
...
const getData = ()=>{
  setLoading(true);
  axios.get(`${process.env.REACT_APP_API_URL}/product/category/66e0abb42e6bda2ea8fee819`)
  .then(res=>{
    setLoading(false);
    ...
    // setdublicateBrand([...new Set(res.data.product.map(product => product.brandName.toLowerCase()).sort())]);
...
{FiltererrorMessage ? (<p id='noMatch'>{'No product matches the filter.'}</p>) :
    ...
    { (filteredProducts.length > 0 ? filteredProducts : mobile).map((eachMobile,index)  => {
      return (
        <div className='chudidar-main-container fridge-main-container' key={eachMobile._id}>
        <div className='fridgeItemImg-container'>
        ...
        <div className='handbagItemDetails-container'>
          ...
          <a className='chudidarItem-name gameItem-name fridgeItem-name' id='handbagItem-name-ID' onClick={()=>navigateSoloLaptop(eachMobile._id)}>{eachMobile.title}</a>
...
<button className='items-Cart' id='items-card-ID' onClick={()=>handleAddtoCart(eachMobile,index)}>{statuscartlabel & cartLoading[index] ? (<SmallLoader/>):(<> {cartlabel}</>)}</button>
       {showADDnew && <div className='upd-del' style={{display:'flex',flexDirection:"column",gap:'10px'}}>
