...
const handleAddtoCart = (productDetails,index) => {
  const selectedSize = document.querySelector(`input[name="chudidarSize-${index}"]:checked`)?.value; 
  if(!selectedSize){
    ...
    { (filteredProducts.length > 0 ? filteredProducts : mobile).map((eachMobile,index)  => {
     ...
     {shoeSizes.map((size) => (
            <div key={size} className='sizeKey'>
                <input type="radio" value={size} name={`chudidarSize-${index}`} id={`size-${index}-${size}`} />
