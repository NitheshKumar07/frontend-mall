...
import '../stylesheets/handbagPage.css'
...
const getData = ()=>{
  ...
  .then(res=>{
    ...
    setfilteredProducts(products); //initially filtered as all products
      ...
      // Set dublicateColour (unchanged)
      setdublicateColour([...new Set(
        res.data.product
          .map(product => product.colour?.toLowerCase()) // Normalize to lowercase
          .filter(colour => colour) // Filter out empty or undefined values
      )]);
...
<div className='laptopBannerDiv'><img id='handbagBanner' src={require('../assests/handbagbanner.jpg')} style={{marginBottom:'1pc'}}/><p>HandBag</p></div>
{ (filteredProducts.length > 0 ? filteredProducts : mobile).map((eachMobile,index)  => {
  return (
    ...
    <div className='handbagItemDetails-container'>
      ...
      <button className={`items-Cart cartbtnFCN-${index}`} id='items-card-ID' 
      onClick={()=>handleAddtoCart(eachMobile,index)}>{statuscartlabel & cartLoading[index] ? (<SmallLoader/>):(<> {cartlabel}</>)}</button>
