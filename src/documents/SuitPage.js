...
import '../stylesheets/suitPage.css'
import { useLocation, useNavigate } from 'react-router';
import SoloLaptop from './SoloLaptop';
...
const handleAddtoCart = (productDetails,index) => {
  const selectedSize = document.querySelector(`input[name="SuitSize-${index}"]:checked`)?.value; 
  if(!selectedSize){
...
{/* <div className='svgfilterDiv1'>
<svg xmlns="http://www.w3.org/2000/svg" onClick={goTop} className='filterSvg' height="26px" width="26px" viewBox="0 -960 960 960"><path d="M440-160q-17 0-28.5-11.5T400-200v-240L168-736q-15-20-4.5-42t36.5-22h560q26 0 36.5 22t-4.5 42L560-440v240q0 17-11.5 28.5T520-160h-80Zm40-308 198-252H282l198 252Zm0 0Z"/></svg>
</div> */}
...
{FiltererrorMessage ? (<p id='noMatch'>{'No product matches the filter.'}</p>) :
  <div className='super-box super-boxChudidar'>
            <img id='sareeBanner' src={require("../assests/suitsBanner.webp")} style={{marginBottom:'1pc'}}/>
    { (filteredProducts.length > 0 ? filteredProducts : mobile).map((eachMobile,index)  => {
      return (
        <div className='chudidar-main-container bag-main-container' key={eachMobile._id}>
        <div className='suitItemImg-container BagsuitItemImg-container'>
        ...
        <div className='handbagItemDetails-container'>
          ...
          <a className='chudidarItem-name bagItem-name' id='handbagItem-name-ID' onClick={()=>navigateSoloLaptop(eachMobile._id)}>{eachMobile.title}</a>
          ...
          <div className='sizeChartALL SuitsizeChartALL'>
        <div className="shoe-sizes-container shoe-sizes-containerALL">
     {shoeSizes.map((size) => (
            <div key={size} className='sizeKey'>
                <input type="radio" value={size} name={`SuitSize-${index}`} id={`size-${index}-${size}`} />
