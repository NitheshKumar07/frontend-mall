...
const SoloJeans = () => {
 ...
 useEffect(() => {
  ...
  axios.get('http://3.110.113.45:5000/product/category/66e0ace82e6bda2ea8fee825')
    ...
    // buy
    const buyNow = () => {
      const selectedSize = document.querySelector('input[name="jeansSize"]:checked')?.value; 
...
const handleAddtoCart = () => {
  ...
  <div className='solojeansImg-Container'>
      <div className='solojeansImg-box'>
  ...
  <div className='soloLaptopDetails-container'>
    ...
    <div className='sizeChart'>
     ...
     {shoeSizes.map((size) => (
            <div key={size}>
                <input type="radio" value={size} name="jeansSize" id={`size${size}`} />
